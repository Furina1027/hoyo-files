...
> （例：`Transfer/HollowLoading/Zero/ChallengeZero02.usm` 是 3440×1080 / 181 帧，  
> `Yorozuya/Zero/ChallengeZero02.usm` 是 1452×590 / 493 帧），而 `{game}_keys.json`  
> 只能按文件名存一把密钥，故上表按唯一文件名计为 2591 个。  
...
- 原神 7.1：支持 VP9 AES-128-CTR 视频与 HCA 音频；`Video_Reunion_67_test.usm` 静默回退到历史 67，`Video_Reunion_71.usm` 使用独立 AES/HCA key
- 密钥库：内置三款游戏视频密钥，随仓库分发
...
- 在线播放：原神 7.1 VP9 走服务器 AES 解密后推送 WebM，旧版 VP9/历史回退走 mask 解密；H.264 走服务器转封装
- 原神 HCA：支持 7.1 AES/HCA；历史 `Video_Reunion_67.usm` 使用旧 HCA key，不与 67_test 混用
- 导出：原神 → MKV（含音轨，可选语言通道）；崩铁 → MP4 / 带音轨 MKV；绝区零 → WebM
- 预下载：chunk 直下优先，失败后回退 diff/hpatchz；预下载 USM 差异可参与历史更新

### 原神 7.1 / Reunion 适配

- `Video_Reunion_67_test.usm` 保留前端显示名；点击播放时后端静默改取 6.7.0 的 `Video_Reunion_67.usm`，视频和 HCA 音频均使用历史源的解密路径。
- `Video_Reunion_71.usm` 使用 7.1 method2：视频 AES-128-CTR，音频 HCA；nonce 从每个 USM 的 `VIDEO_HDRINFO` 读取。
- 7.1 版本清单、chunk/index、USM history 和 key 已随 `pkg_version/` 分发；无需提交本地 `downloads/` 样本。
...
- Node.js 24+ 与 pnpm
- ffmpeg（原神 7.1 VP9 WebM/MKV 播放与导出需要）
...
数据仓库在仓库根目录的 `pkg_version/`（版本构建快照、USM 历史与密钥），
由数据服务器静态托管。修改位置见 `server/server.mjs` 的 `--data` 参数。

`pkg_version/chunk/*.index.json` 是运行时根据构建快照和 manifest 生成的可重建缓存，不随仓库提交；首次使用对应版本的 chunk 下载时会按需生成。
...
`pkg_version/usm/` 下与密钥相关的文件：

| 文件                    | 说明                                                        |
| --------------------- | --------------------------------------------------------- |
| `{game}_keys.json`    | 主密钥库：**纯文件名（去 `.usm`）→ key**；旧版为 16 位 mask 字符串，7.1 为 `{ aes, audio }` 对象；全零 = 明文 |
| `{game}_keys_by_path.json` | **按路径的覆盖表**（可选）：相对视频根且去 `.usm` 的路径 → key                   |
| `{game}_history.json` | USM 历史：按**完整路径**记录各版本状态与 md5/size                          |

取 key 的顺序是 **按路径（覆盖表）→ 按文件名（主库）**。这不是冗余设计：绝区零存在**同名但内容
不同**的视频（`Transfer/HollowLoading/*` 与 `Yorozuya/*` 等成套的装载视频、`Procedure/*` 与
`Activity|C777_EP*` 等），官方给它们下发的是**不同**的 vk，只按文件名存一把必然张冠李戴 ——
即"库里有 key、前端也显示可播放，但解出来是错误的另一个视频"。覆盖表只收录
「路径专属 key ≠ 主库那把」的条目（绝区零约 50 条），官方 `FEJENOLPLMC=0`（未下发 vk）的条目
一律不收，以免把已破解出的真 key 覆盖成"明文"。

本地文件定位同理：按请求路径在视频根内**精确命中**，同名文件不再互相串门；
**热更目录 `Persistent` 仍然优先于 `StreamingAssets`**，只是"根内"改为优先精确路径。
