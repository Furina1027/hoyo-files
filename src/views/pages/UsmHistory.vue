...
const usmRefreshing = ref(false)
const usmRefreshResult = ref<string | null>(null)

/** 对比最新两个版本的 pkg_version 清单, 更新 USM 历史 (新增/删除/修改) */
async function handleRefreshUsmHistory() {
  usmRefreshing.value = true
  usmRefreshResult.value = null
  try {
    const res = await fetch(`${API_BASE}/api/refresh-usm-history/${gameId.value}`, { method: 'POST' })
    const data = await res.json()
    if (data.error)
      throw new Error(data.error)
    usmRefreshResult.value = `已更新: ${data.version} (对比 ${data.prev}) 新增 ${data.added} · 删除 ${data.deleted} · 修改 ${data.changed}`
    await historyQuery.refetch()
  }
  catch (err) {
    usmRefreshResult.value = `更新失败: ${(err as Error).message}`
  }
  finally {
    ...
    <template v-else>
      <div class="flex shrink-0 items-center gap-6 border-b border-gray-200 bg-white px-4 py-3 dark:border-gray-700 dark:bg-gray-900">
        ...
        <div class="ml-auto flex items-center gap-2 text-xs text-gray-400 dark:text-gray-500">
          <button
            class="flex items-center gap-1 rounded-md border border-blue-200 px-2 py-1 text-xs font-medium text-blue-600 transition-colors hover:bg-blue-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-blue-800/50 dark:text-blue-400 dark:hover:bg-blue-900/20"
            :disabled="usmRefreshing"
            @click="handleRefreshUsmHistory"
          >
            <LucideLoader2 v-if="usmRefreshing" class="h-3 w-3 animate-spin" />
            <LucideRefreshCw v-else class="h-3 w-3" />
            {{ usmRefreshing ? '更新中...' : '更新历史' }}
          </button>
          <span v-if="usmRefreshResult" :class="usmRefreshResult.startsWith('更新失败') ? 'text-red-500' : 'text-green-600 dark:text-green-400'">
            {{ usmRefreshResult }}
          </span>
          <span>共 {{ allFiles.length.toLocaleString() }} 条记录</span>
        </div>
      </div>
                  ...
                  <template v-if="entry.label !== '删除'">
                    ...
                    <div class="mt-2 flex flex-wrap gap-1.5">
                      ...
                      <template v-else>
                        ...
                        <button
                          v-if="getEntryKey(selectedFile!.filename)"
